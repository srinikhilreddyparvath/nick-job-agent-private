...
class SemanticAnalysisService:
    ...
    def analyze(self,db:Session,job_id:int,refresh:bool=False)->AnalysisResponse:
        ...
        try:
            ...
            report.strengths=approved;report.gaps,contradicted_gaps=self.validator.filter_contradicted_gaps(report.gaps);report.requirement_gaps,contradicted_requirement_gaps=self.validator.filter_contradicted_gaps(report.requirement_gaps)
            ...
            saved=cached or SemanticAnalysisRecord(job_id=job_id,fingerprint=fingerprint);saved.deterministic_score=deterministic;saved.semantic_score=report.semantic_fit_score;saved.blended_score=report.blended_score;saved.report_json=report.model_dump(mode="json");saved.provider=report.provider;saved.model=report.model;saved.prompt_version=VERSION;saved.evidence_version=version;saved.input_tokens=report.input_tokens;saved.output_tokens=report.output_tokens;saved.cached_tokens=report.cached_tokens;saved.estimated_cost=report.estimated_cost;saved.latency_ms=report.latency_ms;saved.created_at=datetime.now(timezone.utc);db.add(saved);db.commit()
