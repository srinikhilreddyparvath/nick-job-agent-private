...
class HttpPublicResearchProvider(PublicResearchProvider):
    def retrieve(self,urls,limit):
        ...
        for url in urls[:limit]:
            ...
            try:
                response=httpx.get(url,timeout=15,follow_redirects=True,headers={"User-Agent":"NickJobAgent/0.1"});response.raise_for_status();text=re.sub(r"<[^>]+>"," ",response.text);text=" ".join(text.split())[:3000];results.append(ResearchSource(url=str(response.url),title=str(response.url),source_type="public_page",excerpt=text))
...
class ResearchService:
    ...
    def research(self,db:Session,job_id:int,refresh=False):
        ...
        fingerprint=hashlib.sha256(json.dumps({"job":record.description_fingerprint,"urls":urls,"prompt":VERSION,"provider":self.llm.provider.name,"model":self.llm.provider.model},sort_keys=True).encode()).hexdigest();cached=db.scalar(select(CompanyResearchRecord).where(CompanyResearchRecord.fingerprint==fingerprint))
        ...
        try:
            if cached and not refresh:
                report=JobResearchReport.model_validate(cached.report_json);report.cache_hit=True;self.runs.complete(db,run,report.model_dump(mode="json"),[{"step":1,"tool":"research_cache","status":"hit"}],sources_used=[x.url for x in report.source_citations]);return ResearchResponse(status="completed",report=report,agent_run_id=run.id)
            sources=self.provider.retrieve(urls,self.settings.llm_max_research_pages_per_job)
            if not sources:sources=[ResearchSource(url=str(job.source_url),title=f"{job.company} - {job.title}",source_type="stored_job_posting",excerpt=(job.description or job.title)[:3000])]
            context={"job_id":job_id,"company":job.company,"title":job.title,"description":job.description,"requirements":job.requirements,"preferred_requirements":job.preferred_qualifications,"sources":[x.model_dump(mode="json") for x in sources]};report,response=self.llm.generate_structured(SYSTEM,json.dumps(context),JobResearchReport);allowed={x.url for x in sources};report.source_citations=[x for x in report.source_citations if x.url in allowed] or sources;report.provider=response.provider;report.model=response.model;report.prompt_version=VERSION;report.latency_ms=response.latency_ms;report.input_tokens=response.usage.input_tokens;report.output_tokens=response.usage.output_tokens;report.estimated_cost=response.usage.estimated_cost
            saved=CompanyResearchRecord(company_id=company.id if company else None,job_id=job_id,fingerprint=fingerprint,summary=report.company_summary,role_context=report.role_summary,sources=[x.model_dump(mode="json") for x in report.source_citations],report_json=report.model_dump(mode="json"),provider=report.provider,model=report.model,prompt_version=VERSION,input_tokens=report.input_tokens,output_tokens=report.output_tokens,estimated_cost=report.estimated_cost,latency_ms=report.latency_ms);db.add(saved);db.commit();actions=[{"step":1,"tool":"get_job","status":"completed"},{"step":2,"tool":"get_public_pages","status":"completed","source_count":len(sources)},{"step":3,"tool":"generate_structured_research","status":"completed"}];self.runs.complete(db,run,report.model_dump(mode="json"),actions,sources_used=[x.url for x in report.source_citations],input_tokens=report.input_tokens,output_tokens=report.output_tokens,estimated_cost=report.estimated_cost,latency_ms=report.latency_ms);return ResearchResponse(status="completed",report=report,agent_run_id=run.id)
