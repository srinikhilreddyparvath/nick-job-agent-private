...
def test_global_ranking_happens_before_pagination():
    ...
    low=service.create(db,job("low",40));low.family_fit_score=40
    high=service.create(db,job("high",95,days_old=2));high.family_fit_score=95
    middle=service.create(db,job("middle",70));middle.family_fit_score=70;db.commit()
...
def test_persisted_semantic_analysis_drives_job_api_score():
    db=SessionLocal();service=JobService();record=service.create(db,job(score=60));record.family_fit_score=60;db.add(SemanticAnalysisRecord(job_id=record.id,fingerprint="semantic-fixture",deterministic_score=60,semantic_score=90,blended_score=76.5,report_json={"confidence":.9,"strengths":[{"statement":"Grounded match","evidence_ids":["E1"]}],"gaps":[],"requirement_gaps":[]},provider="mock",model="mock-fit",prompt_version="v1",evidence_version="v1"));db.commit()
...
def test_find_jobs_requires_a_source(client):
    response=client.post("/career-intelligence/find-jobs")
    assert response.status_code==409 and "public company career page" in response.json()["detail"]


def test_new_jobs_are_selected_before_already_analyzed_jobs():
    db=SessionLocal();source(db);service=JobService()
    analyzed=service.create(db,job("analyzed",99));analyzed.family_fit_score=99
    fresh=service.create(db,job("fresh",70));fresh.family_fit_score=70
    db.add(SemanticAnalysisRecord(job_id=analyzed.id,fingerprint="old",semantic_score=99,report_json={},provider="mock",model="mock",prompt_version="v1",evidence_version="v1"));db.commit()
...
def test_progress_reports_real_stages_and_semantic_failure_fallback():
    ...
    db=SessionLocal();source(db);record=JobService().create(db,job("fallback",72));record.family_fit_score=72;db.commit()
    settings=Settings(llm_enabled=True,llm_provider="mock",llm_model="mock",llm_max_semantic_analyses_per_scan=1,candidate_profile_path="../data/profile.example.json",candidate_evidence_path="../data/evidence.example.json",candidate_preferences_path="../data/preferences.example.json")
