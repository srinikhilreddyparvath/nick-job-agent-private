...
const familyLabels:Record<string,string>={ALL:"ALL",RESEARCH_AI:"RESEARCH / AI",DATA_SCIENCE:"DATA SCIENCE",PRODUCT_MANAGEMENT:"PRODUCT",UNKNOWN:"UNKNOWN"};
...
function JobCard({job}:{job:Job}){
  ...
  return <article className="job-card">
    <div className="job-card-top"><div className="company-avatar">{job.company.slice(0,2).toUpperCase()}</div><div className="job-heading"><span>{job.company}</span><h2><Link href={`/jobs/${job.id}`}>{job.title}</Link></h2><div className="family-line"><b>{familyLabels[job.role_family]}</b>{opportunity?<em>{opportunity.recommendation.replaceAll("_"," ")}</em>:null}</div></div><div className="score"><strong>{opportunity?.overall_score??job.fit_score??"-"}</strong><span>OPPORTUNITY</span></div></div>
...
export default function Dashboard({data,referenceDate,sources,latestRun}:{data:JobList;referenceDate:string;sources:JobSource[];latestRun:CareerRun|null}){
  ...
  return <div className="page-wrap">
    ...
    <section className="family-tabs">{families.map(x=><button type="button" className={x===family?"active":""} onClick={()=>setFamily(x)} key={x}>{familyLabels[x]}</button>)}</section>
    ...
    <section className="job-list">{jobs.length?jobs.map(x=><JobCard job={x} key={x.id}/>):<div className="empty-state"><Search size={24}/><h2>No opportunities yet</h2><p>Connect a public ATS source or add a job URL to begin.</p></div>}</section>
  </div>;
}
