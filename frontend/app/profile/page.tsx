...
import {getEvidence,getProfile} from "@/lib/api";

export default async function CandidateMemory(){
 const [profile,evidence]=await Promise.all([getProfile(),getEvidence()]);
 if(!profile)return <div className="page-wrap"><div className="empty-state"><h1>Candidate memory unavailable</h1><p>Start the backend to load verified profile evidence.</p></div></div>;
 const grouped=Object.groupBy(evidence,item=>item.company??item.category);
 return <div className="page-wrap memory-page"><header className="topbar"><div><p className="eyebrow">VERIFIED CANDIDATE STATE</p><h1>Candidate memory</h1><p>Traceable facts available to future agents. Read-only in Phase 2.</p></div><div className="memory-count"><Database size={17}/><strong>{evidence.length}</strong><span>evidence records</span></div></header>
